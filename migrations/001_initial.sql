...
CREATE TABLE IF NOT EXISTS settings (
    ...
    scope           TEXT NOT NULL DEFAULT 'global',  -- global | workspace | user
...
CREATE TABLE IF NOT EXISTS sessions (
    ...
    revision        INTEGER NOT NULL DEFAULT 0,      -- optimistic lock for sync
    model_id        TEXT,
...
CREATE TABLE IF NOT EXISTS messages (
    ...
    metadata_json   TEXT,           -- tool_calls, attachments, token_usage
...
-- ---------------------------------------------------------------------------
-- LangGraph checkpoints (pause/resume)
-- ---------------------------------------------------------------------------
CREATE TABLE IF NOT EXISTS checkpoints (
    ...
    parent_id       TEXT,
    state_blob      BLOB NOT NULL,      -- serialized graph state
    metadata_json   TEXT,
    created_at      TEXT NOT NULL DEFAULT (datetime('now'))
);
...
CREATE TABLE IF NOT EXISTS tool_definitions (
    ...
    source          TEXT NOT NULL DEFAULT 'builtin',  -- builtin | mcp | plugin
...
CREATE TABLE IF NOT EXISTS permission_policies (
    ...
    workspace_id    TEXT REFERENCES workspaces(id) ON DELETE CASCADE,
    tool_name       TEXT,
    resource_pattern TEXT,   -- glob path, command prefix, host
...
CREATE TABLE IF NOT EXISTS audit_log (
    ...
    arguments_json  TEXT,
    result_status   TEXT NOT NULL,  -- ok | denied | error | pending_approval
    approved_by     TEXT,           -- user | policy | null
    duration_ms     INTEGER,
    created_at      TEXT NOT NULL DEFAULT (datetime('now'))
);
...
-- ---------------------------------------------------------------------------
-- Code index (Tree-sitter) — vector refs in Chroma, metadata here
-- ---------------------------------------------------------------------------
CREATE TABLE IF NOT EXISTS code_chunks (
    ...
    chroma_id       TEXT,           -- external vector id
...
-- ---------------------------------------------------------------------------
-- Sync state (CLI <-> Desktop)
-- ---------------------------------------------------------------------------
CREATE TABLE IF NOT EXISTS sync_cursors (
    client_id       TEXT PRIMARY KEY,  -- cli | desktop:<uuid>
